...
export function generateAnchor(ir: ProgramIR): {
  ...
  files.push({
    ...
    content: generateLibRs(programName, programId, instructions, states, errors_, events),
    language: 'rust',
  });
...
function generateLibRs(
  ...
  const ixLines = instructions.map((ix) => {
    const ctx = toPascalCase(ix.name);
    const args = ix.args.map((a) => `${a.name}: ${solanaTypeToRust(a.type)}`).join(', ');
...
function generateInstructionRs(
  ...
  for (const s of [...usedStates].sort()) importLines.push(`use crate::state::${s};`);
  if (hasErrors && ir.errors.length > 0) importLines.push(`use crate::errors::${errorEnum};`);
  for (const e of [...usedEvents].sort()) importLines.push(`use crate::events::${e};`);

  // Build instruction body
  const bodyLines = generateInstructionBody(ix);

  // Build args signature
  const argSig  = ix.args.map((a) => `${a.name}: ${solanaTypeToRust(a.type)}`).join(', ');
...
function emitLogicOp(op: LogicOperation): string[] {
  switch (op.type) {
    ...
    case 'if-else': {
      ...
      const else_ = op.elseBody?.flatMap(emitLogicOp).map((l) => `    ${l}`) ?? [];
    ...
    case 'math': {
      ...
      return [`let ${op.result} = ${op.left} ${opMap[op.operation]} ${op.right};`];
    }
    ...
    case 'cpi': {
      ...
      return [
        ...
        `// TODO: build CPI context for ${op.targetProgram}`,
        `// Accounts: { ${accountMappings.trim()} }`,
        `// Args: ${dataArgs || 'none'}`,
      ];
    }
...
function buildAccountField(account: Account, ir: ProgramIR): string {
  const lines: string[] = [];
  const attrs = buildAccountAttributes(account, ir);
...
function buildAccountAttributes(account: Account, _ir: ProgramIR): string[] {
  ...
  for (const c of constraints) {
    switch (c.type) {
      ...
      case 'realloc': {
        ...
        parts.push(`realloc = ${c.space}, realloc::payer = ${c.payer}, realloc::zero = ${z}`);
        break;
      }
      case 'custom': {
        const err = c.errorCode ? ` @ ${c.errorCode}` : '';
...
function accountToRustType(account: Account): string {
  switch (account.accountType) {
    case 'signer':                 return `Signer<'info>`;
    case 'system-account':         return `SystemAccount<'info>`;
    case 'system-program':         return `Program<'info, System>`;
    case 'token-program':          return `Program<'info, anchor_spl::token::Token>`;
    case 'associated-token-program': return `Program<'info, anchor_spl::associated_token::AssociatedToken>`;
    case 'rent':                   return `Sysvar<'info, Rent>`;
    case 'clock':                  return `Sysvar<'info, Clock>`;
    case 'mint':                   return `Account<'info, anchor_spl::token::Mint>`;
    case 'token-account':          return `Account<'info, anchor_spl::token::TokenAccount>`;
    case 'associated-token':       return `Account<'info, anchor_spl::token::TokenAccount>`;
    case 'unchecked-account':      return `/// CHECK: validated by constraint\n    UncheckedAccount<'info>`;
    case 'program':
...
function generateStateRs(name: string, fields: Field[]): string {
  ...
  const derive = hasDynamic
    ? '#[account]\n#[derive(InitSpace)]'
    : '#[account]';

  const fieldLines = fields.map((f) => {
    const rustType  = solanaTypeToRust(f.type);
    const maxLenAttr = hasDynamic && f.maxLen != null ? `    #[max_len(${f.maxLen})]\n` : '';
...
function generateErrorsRs(
  ...
  const variants = errors.map((e) => `    #[msg("${e.message}")]\n    ${e.name},`).join('\n');
  return `use anchor_lang::prelude::*;
